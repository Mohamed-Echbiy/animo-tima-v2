...
export const getStaticProps = async () => {
  ...
  const resP1 = await fetch(
    `${process.env.HOSTNAME}advanced-search?status=FINISHED&perPage=100&page=1`
  );
  const dataP1 = await resP1.json();
  const result1 = await dataP1.results;
  //-------------------------
  const resP2 = await fetch(
    `${process.env.HOSTNAME}advanced-search?status=FINISHED&perPage=100&page=2`
  );
  const dataP2 = await resP2.json();
  const result2 = await dataP2.results;
  //-------------------------
  const resP3 = await fetch(
    `${process.env.HOSTNAME}advanced-search?status=FINISHED&perPage=100&page=3`
  );
  const dataP3 = await resP3.json();
  const result3 = await dataP3.results;
  //-------------------------
  const resP4 = await fetch(
    `${process.env.HOSTNAME}advanced-search?status=FINISHED&perPage=100&page=4`
  );
