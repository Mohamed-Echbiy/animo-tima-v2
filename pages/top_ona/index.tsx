...
export const getStaticProps = async () => {
  const resP1 = await fetch(`https://api.jikan.moe/v4/top/anime?type=ona`);
  const dataP1 = await resP1.json();
  const result1 = await dataP1.data;
  const resP2 = await fetch(
    `https://api.jikan.moe/v4/top/anime?type=ona&page=2`
  );
  const dataP2 = await resP2.json();
  const result2 = await dataP2.data;
  const resP3 = await fetch(
    `https://api.jikan.moe/v4/top/anime?type=ona&page=3`
  );
